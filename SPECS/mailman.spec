...
Summary: Mailing list manager with built in Web access (by dsoares)
Name: mailman
Version: 2.1.26
Release: 1%{?dist}
Epoch: 3
...
#dsoares %global contentdir /usr/share/httpd
...
#dsoares mkdir -p %{buildroot}/%{_tmpfilesdir}
#dsoares install -m 0644 %{SOURCE9} %{buildroot}%{_tmpfilesdir}/%{name}.conf

# Systemd service file
#dsoares mkdir -p %{buildroot}%{_unitdir}
#dsoares install -m644 %{SOURCE10} %{buildroot}%{_unitdir}
mkdir -p %{buildroot}%{_initddir}
install -m755 %{SOURCE11} %{buildroot}%{_initddir}/mailman
...
# systemd
#dsoares %systemd_post mailman.service
...
#dsoares %systemd_preun mailman.service
# rpm should not abort if last command run had non-zero exit status, exit cleanly
exit 0
...
# systemd
#dsoares %systemd_postun_with_restart mailman.service
...
/sbin/chkconfig --add mailman >/dev/null 2>&1 || :
...
#dsoares %{_unitdir}/mailman.service
%{_initddir}/mailman
%doc %{docdir}
...
#dsoares %{_tmpfilesdir}/%{name}.conf
...
%changelog
* Tue Jun 12 2018 Diana Soares <dsoares@fc.up.pt> - 3:2.1.26-1
- Hacked previous source rpm to use mailman version 2.1.26
  (features ReCaptcha, FORM_SECRET, ...)
